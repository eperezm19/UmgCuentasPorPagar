...
async function LlenarTabla() {
    ...
    list.forEach(function (elemento, indice, array) {
        ...
        if (indice == 0) {
            ...
            $('#dtDetalles').append('<thead> <tr> <th>No. </th> <th>Orden de Compra</th> <th>Producto</th> <th>Cantidad</th> <th>Importe</th> <th>Operaciones</th> </tr> </thead>' +
                ' <tbody></tbody>');
        ...
        if (elemento.ordenCompraId == id) {
            ...
            Acum += elemento.importe;

            $('#dtDetalles').append('<tr> <td>' + elemento.ordenCompraDetalleId + '</td> <td>' + elemento.ordenCompraId + '</td> <td>' + productoDesc + '</td> <td>' + elemento.cantidad + '</td> <td>' + parseFloat(elemento.importe).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,') + '</td> <td> ' + boton + ' </td> </tr>');
...
async function AgregarProducto () {
    ...
    if (id > 0) {
        ...
        if (productoId > 0 && cantidad > 0) {
            ...
            let url = host + 'api/OrdenCompraDetalles';
            var respuesta = null;
            var importe = objProducto.costoUnit * cantidad;
            ...
            fetch(url, {
                ...
                body: JSON.stringify({
                    "OrdenCompraId": id, "ProductoId": productoId, "Cantidad": cantidad, "Importe": importe,
                }),
                cache: 'no-cache'
            })
                .then(function (response) {
...
async function RecuperarProductosDisp() {
    ...
    let url = host + 'api/ProductoProveedores';
